...
public final class InnerContainerStruct {
  public enum Fields {
    ...
    INNER_AS_BYTES_VALUE("inner_as_bytes_value"),
    INNER_AS_BYTES_VALUE_STR("inner_as_bytes_value_str");
    ...
    public static Iterable<Fields> all() {
      ...
      b.add(INNER_AS_BYTES_VALUE);
      b.add(INNER_AS_BYTES_VALUE_STR);
      return b.build();
    }
  }
  ...
  private static io.structs.testing.TestingProto.InnerContainer ofRow(Row row) throws com.google.protobuf.InvalidProtocolBufferException {
    ...
    {
      int idx = row.getColumnDefinitions().getIndexOf("inner_as_bytes_value");
      if (!row.isNull(idx)) {
        int value = row.getInt(idx);
        b.getInnerAsBytesBuilder().setValue(value);
      }
    }

    {
      int idx = row.getColumnDefinitions().getIndexOf("inner_as_bytes_value_str");
      if (!row.isNull(idx)) {
        String value = row.getString(idx);
        b.getInnerAsBytesBuilder().setValueStr(value);
      }
    }
  ...
  private static PreparedStatement createInsertAllStmt(Session session) {
    ...
    sb.append("id, inner_value, inner_value_str, inner_as_bytes_value, inner_as_bytes_value_str) ");
    sb.append("VALUES (?, ?, ?, ?, ?)");
  ...
  public void save(io.structs.testing.TestingProto.InnerContainer obj) {
    ...
    {
      Object o = null;
      o = obj.getInnerAsBytes().getValue();
      boundObjs[3] = o;
    }

    {
      Object o = null;
      o = obj.getInnerAsBytes().getValueStr();
      boundObjs[4] = o;
    }
